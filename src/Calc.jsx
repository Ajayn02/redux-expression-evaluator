    ...
    return (
        <>
            <div className='container-fluid d-flex justify-content-center align-items-center' style={{ height: "90vh" }}>
                <div className=' p-5 border rounded border-2  shadow'>
                    <h3 className='text-center mb-3'>Calculate</h3>
                    <input type="text" className='form-control mb-3 me-3' placeholder='Enter an expression' onChange={(e)=>{setVal(e.target.value)}} />
                    
                        <button className='btn btn-success mb-3' onClick={()=>{dispatch(calculate(val))}}>Result</button>
                        <button className='btn btn-warning ms-3 ' onClick={()=>{dispatch(clear())}} style={{marginTop:"-15px"}}>Reset</button>
